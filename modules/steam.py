...
class Steam:
    ...
    def gather_app_info(self):
        ...
        for _app in self.apps:
            ...
            key = _app.id + ":" + _app.filter
            pic_name=_product_info.get("apps").get(int(_app.id)).get("common").get("header_image").get("english")
            pic_url = f'https://cdn.cloudflare.steamstatic.com/steam/apps/{_app.id}/{pic_name}'
            self.new_result[key] = Result(
                ...
                data=_product_info.get('apps', {}).get(int(_app.id), {}).get('depots', {}).get('branches', {}).get(_app.filter, {}).get('timeupdated', {}),
